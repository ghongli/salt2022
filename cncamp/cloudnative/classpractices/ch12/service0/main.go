...
func main() {
	...
	klogv2.Infof("Starting service0")
...
func rootHandler(w http.ResponseWriter, r *http.Request) {
	...
	req, err := http.NewRequest(http.MethodGet, "http://service1", nil)
	if err != nil {
		klogv2.Errorf("http new request err: %s", err)
	}
	...
	klogv2.Infof("lower case header: %v", lowerCaseHeader)
	req.Header = lowerCaseHeader

	client := &http.Client{}
	resp, err := client.Do(req)
	if err != nil {
		klogv2.Info("HTTP get failed with err: ", "error", err)
	} else {
		klogv2.Info("HTTP get successfully")
	}
