...
可以将视图理解为存储起来的select语句，本身并不会保存数据；视图的创建、删除只影响视图本身，不影响对应的基表；数据真正保存在数据表中，当对视图中的数据进行增加、删除和修改时，数据表中的数据会相应地发生变化。
...
--- UNDEFINED 自动选择算法
--- MERGE 表示将视图设置的字段名称和select语句从表中获取的字段的内容合并
...
-- 视图的信息存储在information_schema数据库下的views表中
...
4. 在定义视图的SELECT语句后的字段列表中使用了`数学表达式或子查询`，视图将不支持INSERT，也不支持UPDATE使用了数学表达式、子查询的字段值；
...
区别：
1. 视图是已经编译好的SQL语句的结果集，是基于SQL语句结果集的可视化的虚拟表，属于局部模式的表，而表是实表，属于全局模式的表；
2. 视图没有实际的物理记录，而表有；
3. 表是内容，视图是窗口；
4. 表和视图虽然都占用物理空间，但视图只是逻辑概念；
5. 视图是查看表数据的一种方法，可以查询数据表中某些字段构成的数据，只是一些SQL语句的集合；
6. 从安全角度来说，视图可以防止用户接触数据表，因而不知道表结构；
7. 视图的建立和删除只影响视图本身，而不影响对应的基本表；

联系：
视图是在基本表上建立，结构(所定义的列)和内容(所有记录)都来自于基本表，依赖基本表存在而存在，一个视图可以对应一个或多个基本表。
